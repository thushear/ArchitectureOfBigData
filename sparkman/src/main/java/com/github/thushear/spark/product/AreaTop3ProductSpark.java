...
import com.github.thushear.spark.dao.IAreaTop3ProductDAO;
...
public class AreaTop3ProductSpark {
  ...
  public static void main(String[] args) {
    ...
    long taskId =
      ParamUtils.getTaskIdFromArgs(args,Constants.SPARK_LOCAL_TASKID_PRODUCT);
  ...
  private static JavaRDD<Row> getAreaTop3ProductRDD(SQLContext sqlContext) {
    ...
    sql =
      "SELECT "
        + "t.area,"
        + "CASE "
        + "WHEN t.area='华北' OR area='华东' THEN 'A级' "
        + "WHEN t.area='华南' OR area='华中' THEN 'B级' "
        + "WHEN t.area='西北' OR area='西南' THEN 'C级' "
        + "ELSE 'D级' "
        + "END area_level,"
        + "t.product_id,"
        + "t.click_count,"
